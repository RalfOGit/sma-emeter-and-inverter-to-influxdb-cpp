...
int SpeedwireSocket::openSocketV6(const std::string &local_interface_address) {
    ...
    // if address is INADDR_ANY, the socket will receive unicast traffic from any local interface;
    // for windows hosts: if address is a local interface address, the socket will receive unicast and multicast traffic from that local interface;
    // for linux hosts: if address is a local interface address, the socket will receive just unicast from that local interface but no multicast traffic    struct sockaddr_in6 saddr;
